...
public class FileUploadController {
	...
	@RequestMapping(value = "/output/{path}", method = RequestMethod.HEAD)
	public void getFileInfo(@PathVariable("path") String filePath, HttpServletRequest req, HttpServletResponse res) {
		try {
			processRequest(OUTPUT_FOLDER_PATH + filePath, req, res, false);
		} catch (IOException e) {
			logger.warn("Exception occured on I/O level", e);
		}
	}
	...
	public void getFile(@PathVariable("path") String filePath, HttpServletRequest req, HttpServletResponse res) {
		try {
			processRequest(OUTPUT_FOLDER_PATH + filePath, req, res, true);
		} catch (IOException e) {
			logger.warn("Exception occured on I/O level", e);
		}
	}

	@RequestMapping(value = "/output/compiled/{path}", method = RequestMethod.HEAD)
	public void getCompiledFileInfo(@PathVariable("path") String filePath, HttpServletRequest req,
			HttpServletResponse res) {
		try {
			processRequest(COMPILED_OUTPUT_FOLDER_PATH + filePath, req, res, false);
		} catch (IOException e) {
			logger.warn("Exception occured on I/O level", e);
		}
	}

	@RequestMapping(value = "/output/compiled/{path}", method = RequestMethod.GET)
	public void getCompiledFile(@PathVariable("path") String filePath, HttpServletRequest req,
			HttpServletResponse res) {
		try {
			processRequest(COMPILED_OUTPUT_FOLDER_PATH + filePath, req, res, true);
		} catch (IOException e) {
			logger.warn("Exception occured on I/O level", e);
		}
	}
	...
	private void processRequest(String filePath, HttpServletRequest request, HttpServletResponse response,
		...
		File f = new File(context.getRealPath(filePath) + AUDIO_FORMAT);
		if (!f.exists()) {
			logger.error("Could not find file " + context.getRealPath(filePath) + AUDIO_FORMAT);
    ...
    private static void close(Closeable resource) {
        if (resource != null) {
            try {
            	...
            	logger.warn("Exception occured while closing resource", ignore);
            }
        }
    }
